...
cons_opinion_pull <- function(hyperlink, title) {
  ...
  opinion_text <-
    ...
    rvest::html_text2() |>
    dplyr::as_tibble()
  csv_title <- stringr::str_c("conservative_opinions/", title, ".csv")
...
lib_opinion_pull <- function(hyperlink, title) {
  ...
  opinion_text <-
    ...
    rvest::html_text2() |>
    dplyr::as_tibble()
  csv_title <- stringr::str_c("liberal_opinions/", title, ".csv")
  readr::write_csv(opinion_text, file = csv_title)
  return(opinion_text)
}
...
links_cases <- read_csv("court_cases_links_all.csv", show_col_types = FALSE)
...
write_csv(links_cases_final, "links_cases_final.csv")
```

```{r}
# Reading in Corgis Data
supreme_court <- read_csv("supreme_court.csv")
...
<https://corgis-edu.github.io/corgis/csv/supreme_court/>
Source for "supreme_court" read in data. A series of information about Supreme Court Cases that are not the opinions of the case but the summary information of the case
...
final_links <- read_csv("links_cases_final.csv")
...
write_csv(final, "sc_edit_joined.csv")
...
```{r}
# Pulling Conservative Opinions into a list as well as writing them to Disk
cons_text <- clusterApply(cl, purrr::map2(.x = cons_links, .y = cons_docket, .f = cons_opinion_pull))

# Pulling Liberal Opinions into a list as well as writing them to a Disk
lib_text <- clusterApply(cl, purrr::map2(.x = lib_links, .y = lib_docket, .f = lib_opinion_pull))
```
